"""Create an activation-aware SVD FP32 rank sweep from the baseline checkpoint."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=PROJECT_ROOT / "checkpoints" / "act_svd",
    )
    parser.add_argument(
...
def build_act_svd_model(
    ...
    model.load_state_dict(state_dict)
    model.apply_act_svd(covariances, rank_ratio)
    return model
...
def collect_covariances(
    ...
    calibrator.register(model, module_types=(nn.Linear, nn.MultiheadAttention))
    model.eval()
    progress = tqdm(loader, desc="Calibrating Act-SVD", leave=False)
...
def main() -> None:
    ...
    for rank_ratio in args.rank_ratios:
        print(f"[*] Building Act-SVD rank_ratio={rank_ratio:g}")
        model = build_act_svd_model(state_dict, covariances, rank_ratio)
        checkpoint_name = f"act_svd_rank_ratio_{ratio_slug(rank_ratio)}.pth"
