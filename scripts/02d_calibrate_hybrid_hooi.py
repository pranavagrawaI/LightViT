"""Create a hybrid FP32 sweep using optimized Tucker QKV."""
...
from models.tucker_vit import (
    HybridTuckerLightViT,
    TuckerSelfAttention,
    tucker_qkv_relative_error,
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--tucker-backend", choices=("torch", "tensorly"), default="torch"
    )
    parser.add_argument("--hooi-iterations", type=int, default=8)
    parser.add_argument("--tensorly-tol", type=float, default=1e-5)
...
def collect_covariances(
    ...
    model.eval()
    progress = tqdm(loader, desc="Calibrating Hybrid HOOI MLPs", leave=False)
...
def qkv_reconstruction_stats(
    model: HybridTuckerLightViT, state_dict: dict[str, torch.Tensor]
) -> tuple[float, float]:
    errors = []
    for layer_idx, block in enumerate(model.blocks.layers):
        if isinstance(block.self_attn, TuckerSelfAttention):
            weight = state_dict[f"blocks.layers.{layer_idx}.self_attn.in_proj_weight"]
            errors.append(
                tucker_qkv_relative_error(
                    weight,
                    block.self_attn.num_heads,
                    block.self_attn.core.detach().cpu(),
                    block.self_attn.factor_heads.detach().cpu(),
                    block.self_attn.factor_out.detach().cpu(),
                    block.self_attn.factor_in.detach().cpu(),
                )
            )
    if not errors:
        return 0.0, 0.0
...
def main() -> None:
    ...
    if args.hooi_iterations <= 0:
        raise ValueError("hooi-iterations must be positive.")
    if args.tensorly_tol <= 0:
        raise ValueError("tensorly-tol must be positive.")

    tucker_method = "tensorly" if args.tucker_backend == "tensorly" else "hooi"
    if args.output_dir is None:
        output_name = "hybrid_tensorly" if tucker_method == "tensorly" else "hybrid_hooi"
        args.output_dir = PROJECT_ROOT / "checkpoints" / output_name
    ...
    print(f"[*] Tucker backend: {args.tucker_backend}")
    print(f"[*] Tucker iterations: {args.hooi_iterations}")
    ...
    rows.append(
        {
            ...
            **format_metrics_row(baseline_metrics),
            ...
            "tensorly_tol": args.tensorly_tol if tucker_method == "tensorly" else "",
            "qkv_recon_error_mean": "",
            "qkv_recon_error_max": "",
        }
    )
    ...
    for rank_ratio in args.rank_ratios:
        print(f"[*] Building Hybrid {tucker_method} rank_ratio={rank_ratio:g}")
        model = build_hybrid_hooi_model(
            state_dict,
            covariances,
            rank_ratio,
            args.head_rank,
            tucker_method,
            args.hooi_iterations,
            args.tensorly_tol,
        )
        qkv_error_mean, qkv_error_max = qkv_reconstruction_stats(model, state_dict)
        checkpoint_name = f"hybrid_{tucker_method}_rank_ratio_{ratio_slug(rank_ratio)}.pth"
        ...
        torch.save(
            {
                ...
                "compression": {
                    ...
                    "head_rank": args.head_rank or "full",
                    "qkv_recon_error_mean": qkv_error_mean,
                    "qkv_recon_error_max": qkv_error_max,
                },
            },
            checkpoint_path,
        )
        ...
        rows.append(
            {
                ...
                **format_metrics_row(metrics),
                ...
                "hooi_iterations": args.hooi_iterations,
                "tensorly_tol": args.tensorly_tol if tucker_method == "tensorly" else "",
                "qkv_recon_error_mean": f"{qkv_error_mean:.6f}",
                "qkv_recon_error_max": f"{qkv_error_max:.6f}",
            }
        )
